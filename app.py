        ...
        if pd.notnull(rsid_val):
            ...
            for col in freq_cols:
                ...
                try:
                    ...
                    num_val = 0.0
                freq_values[col] = num_val
            
            # 判断是否有任何人群频率大于 0
            has_freq = any(v > 0 for v in freq_values.values())
            ...
            with tab1:
                if has_freq:
                    # 细化地图分布点
                    map_data = [
                        {'Region': '非洲 (African)', 'Lat': 0, 'Lon': 20, 'Freq': freq_values['af_afr']},
                        {'Region': '美洲 (Latino American)', 'Lat': 15, 'Lon': -90, 'Freq': freq_values['af_amr']},
                        {'Region': '东亚 (East Asian)', 'Lat': 35, 'Lon': 110, 'Freq': freq_values['af_eas']},
                        {'Region': '西欧 (Non-Finnish European)', 'Lat': 48, 'Lon': 5, 'Freq': freq_values['af_nfe']},
                        {'Region': '芬兰 (Finnish)', 'Lat': 62, 'Lon': 26, 'Freq': freq_values['af_fin']},
                        {'Region': '南亚 (South Asian)', 'Lat': 22, 'Lon': 78, 'Freq': freq_values['af_sas']},
                        {'Region': '德系犹太人 (Ashkenazi Jewish)', 'Lat': 32, 'Lon': 35, 'Freq': freq_values['af_asj']},
                        {'Region': '其他人群 (Other)', 'Lat': -20, 'Lon': 140, 'Freq': freq_values['af_oth']}
                    ]
                    ...
                    if not valid_m_df.empty:
                        ...
                        # 优化 marker size：使用基础大小 + 比例缩放，确保极低频率也可见
                        # 对于稀有变异（Freq很小），我们设定一个最小直径 10
                        marker_sizes = [12 + (f * 1000) if f < 0.01 else 20 + (f * 100) for f in valid_m_df['Freq']]
                        
                        fig.add_trace(go.Scattergeo(
                            lat=valid_m_df['Lat'],
                            lon=valid_m_df['Lon'],
                            text=valid_m_df['Region'] + "<br>Freq: " + valid_m_df['Freq'].apply(lambda x: f"{x:.6f}"),
                            marker=dict(
                                size=marker_sizes,
                        ...
                        fig.update_layout(
                            title=f"RSID: rs{int(rsid_val)} 的全球人群分布 (仅显示频率 > 0 的地区)",
                            geo=dict(
                        ...
                        st.warning("📊 gnomAD 数据库中暂无该位点的有效人群频率记录。")
                else:
